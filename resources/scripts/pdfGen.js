async function generatePDF()
{
    ...
    await waitForPlotR().then(async plots =>{
        var yLine = 55;
        var imgHeight = 65;
        var pageHeight = 257;
        if(plots['frequency'][0].length == 1)
        {
            await loadImage(plots['frequency']).then(formatedImage =>{
                doc.addImage(formatedImage, 25, yLine, 75, 75);
                console.log("Add image: Frequency plot " + plots['frequency'][0].length)
            })
        }
        else
            for(i = 0; i < plots['frequency'].length; i++)
            {
                await loadImage(plots['frequency'][i]).then(formatedImage =>{
                    doc.addImage(formatedImage, 25, yLine, 75, 75);
                    console.log("Add image: Frequency plot " + i)
                })
                yLine += (imgHeight + 1);
...
function loadImage(src)
{
    return new Promise((resolve, reject) => {
        console.log('Rendering in 4k resolution...')
        let img = new Image(),
            canvas = document.createElement("canvas"),
            ctx = canvas.getContext("2d");
            // upscale the canvas content
            canvas.width = 3840 * devicePixelRatio;
            canvas.height = 2160 * devicePixelRatio;
